# PREFIX=ak8_MD_inclv8std_nonmd.test
PREFIX=ak8_MD_inclv8std_nonmd_test.test2
# PREFIX=ak8_MD_inclv8std_nonmd_100epoch.100epochs_lowlr
# PREFIX=ak8_MD_inclv8std_nonmd_100epoch_withEnergy.100epochs_lowlr
...
modelftopts="-o num_nodes 28 -o num_cls_nodes 28 -o label_cls_nodes ${label_cls_nodes} -o loss_gamma 0 -o use_external_fc True -o fc_params [(256,0),(256,0)] --num-epochs 100 --load-model-weights ${load_model} --exclude-model-weights part\\.fc.* --freeze-model-weights (input_embeds|part\\.(embed|pair_embed|blocks|cls_token|cls_blocks|norm)).* " # the fine-tuned model setup; may override some modelopts

trainopts="--run-mode train,val --num-workers 5 --fetch-step 1. --data-split-group 125 "
testopts="--run-mode test --num-workers 3 --data-split-group 1 " # fetch-by-file

source scripts/train_GloParT_nonMD.sh run 2 --network-config networks/example_ParticleTransformer2024PlusTagger_unified.py --batch-size 512 --start-lr 2e-5 $modelopts $modelftopts $trainopts
source scripts/train_GloParT_nonMD.sh dryrun 0 --network-config networks/example_ParticleTransformer2024PlusTagger_unified.py --batch-size 512 --start-lr 2e-5 $modelopts $modelftopts $testopts
